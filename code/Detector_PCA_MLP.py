...
class Detector_PCA_MLP:
    ...
    def finish_fit(self, benign_labels, *, plot_roc_graph=False, roc_graph_file_name=''):
        ...
        for i in range(len(self.__benign_activation_spaces[0])):
            neigh = MLP(solver='adam', alpha=1e-7, hidden_layer_sizes=(160, 100, ), random_state=0, max_iter=400, activation='identity')
            neigh.fit([x[i] for x in self.__benign_activation_spaces], benign_labels)
            self.mlp_classefiers.append(neigh)

        # Assign input samples with a sequence of class labels
        Y_train_predict = []
        for x in self.__benign_activation_spaces:
            Y_train_predict.append([self.mlp_classefiers[i].predict([x[i]])[0] for i in range(len(x))])
        ...
        for x in self.__adversarial_activation_spaces:
            Y_adversarial_predict.append([self.mlp_classefiers[i].predict([x[i]])[0] for i in range(len(x))])
    ...
    def fit(self, benign_trainset, adv_trainset, benign_labels, *, plot_roc_graph=False, roc_graph_file_name=''):
        ...
        for i in range(len(self.benign_activation_spaces[0])):
            neigh = MLP(solver='adam', alpha=1e-5, hidden_layer_sizes=(50,), random_state=1, batch_size=32, activation='tanh')
        ...
        for x in self.adversarial_activation_spaces:
            Y_advertise_predict.append([self.mlp_classefiers[i].predict([x[i]])[0] for i in range(len(x))])
    ...
    def predict(self, sample):
        ...
        for i in range(len(sample)):
            ...
            sample_layer_labels[i] = self.mlp_classefiers[i].predict([activation_space])
    ...
    def load(self, data_dir_name, file_name):
        with open(os.path.join(data_dir_name, file_name), 'rb') as f:
            ...
            self.mlp_classefiers = d.mlp_classefiers
            self.p_arr = d.p_arr
